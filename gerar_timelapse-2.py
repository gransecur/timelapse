...
DOWNLOADS_SIMULTANEOS = 8     # quantas fotos baixar em paralelo
...
# Cada thread precisa da própria conexão: a biblioteca do Google não é
# segura para uso simultâneo entre threads.
DADOS_CONTA = None
...
def sessao_da_thread():
    if not hasattr(_local, 'sessao'):
        cred = service_account.Credentials.from_service_account_info(
            DADOS_CONTA, scopes=ESCOPOS)
        _local.sessao = AuthorizedSession(cred)
...
def baixar_foto(foto_id, destino):
    """Baixa uma foto usando a conexão própria desta thread."""
    url = (f'https://www.googleapis.com/drive/v3/files/{foto_id}'
           f'?alt=media&supportsAllDrives=true')
    for tentativa in range(3):
        try:
            resposta = sessao_da_thread().get(url, stream=True, timeout=120)
...
def enviar_para_drive(drive, caminho, pasta_destino_id, nome):
    """Tenta enviar o vídeo ao Drive. Devolve None se não for possível.

    Contas de serviço não têm cota de armazenamento própria, então em contas
    Google pessoais o envio falha. Nesse caso o vídeo continua disponível
    como anexo da execução no GitHub.
    """
    midia = MediaFileUpload(caminho, mimetype='video/mp4', resumable=True,
    ...
    try:
        ...
        texto = str(erro)
        log('')
        if 'storageQuotaExceeded' in texto or 'storage quota' in texto:
            log('⚠️  Não foi possível salvar no Google Drive.')
            log('    Contas de serviço não possuem espaço de armazenamento próprio,')
            log('    então elas leem o Drive mas não conseguem criar arquivos nele.')
        else:
            log(f'⚠️  Falha ao enviar para o Drive: {texto[:300]}')
        log('')
        log('    O vídeo está pronto e disponível em "Artifacts", no topo')
        log('    desta página da execução.')
        return None
...
def montar_video(drive, escolhidas, cfg, trabalho):
    ...
    for comeco in range(0, total, FRAMES_POR_LOTE):
        ...
        with ThreadPoolExecutor(max_workers=DOWNLOADS_SIMULTANEOS) as pool:
            ok = list(pool.map(lambda fid: baixar_foto(fid, destinos[fid]), unicas))
...
def ler_configuracao():
    ...
    return {
        ...
        'preset': preset,
        'salvar_no_drive': normalizar(env('SALVAR_NO_DRIVE', 'nao')) in ('sim', 's', 'yes', 'true'),
    }
...
def main():
    ...
    try:
        ...
        # O vídeo vai para a pasta que o GitHub publica como Release
        ...
        duracao_final = fmt_duracao(len(escolhidas) / FPS)
        periodo = f'{cfg["data_inicio"]} a {cfg["data_fim"]}'

        # Notas que aparecem na página do Release
        with open(os.path.join(destino_anexo, 'NOTAS.md'), 'w', encoding='utf-8') as f:
            f.write(f'**Obra:** {nome_obra}\n\n')
            f.write(f'**Período:** {periodo}\n\n')
            f.write(f'**Duração:** {duracao_final}\n\n')
            f.write(f'**Resolução:** {cfg["largura"]}x{cfg["altura"]}\n\n')
            f.write(f'**Tamanho:** {tamanho:.1f} MB\n\n')
            f.write(f'**Fotos usadas:** {len(escolhidas)} frames\n\n')
            f.write('---\n\n')
            f.write('Baixe o arquivo `.mp4` logo abaixo.\n')

        # Informa o nome do arquivo para o passo seguinte do GitHub
        ambiente = os.environ.get('GITHUB_ENV')
        if ambiente:
            with open(ambiente, 'a', encoding='utf-8') as f:
                f.write(f'NOME_VIDEO={cfg["nome_arquivo"]}\n')
                f.write(f'TITULO_RELEASE={nome_obra} | {periodo} | {duracao_final}\n')

        enviado = None
        if cfg['salvar_no_drive']:
        ...
        if enviado:
            log(f'  Drive ...... {enviado.get("webViewLink", "(salvo)")}')
        log('  Download ... veja o Release criado para esta execução')
        ...
        if resumo:
            with open(resumo, 'a') as f:
                ...
                if enviado:
                    ...
                    f.write('Baixe em **Artifacts**, no topo desta pagina.\n')
    finally:
